...
export default function ProfileScreen() {
 ...
 const animatedBadgeStyle = (index: any) =>
  useAnimatedStyle(() => ({
   ...
   transform: [
    {
     scale: withSpring(badgeAnim[index].value, { stiffness: 120, damping: 12 }),
    },
   ],
  }));
 ...
 const handleSettingAction = (action: any) => {
  switch (action) {
   case "callSupport":
    call({ number: "9059875310", prompt: false }).catch((error: any) =>
   ...
   case "referFriend":
    Alert.alert(
     "Refer a Friend",
     "Invite a friend and earn bonus travel points!",
     [
      { text: "Cancel", style: "cancel" },
 ...
 const handleImageUpload = () => {
  Alert.alert(
   "Upload Image",
   "Would you like to upload a new profile or background image?",
   [
    { text: "Cancel", style: "cancel" },
  ...
  >
   ...
   <Animated.View style={[styles.headerContainer, animatedStyle]}>
    ...
    <View style={styles.profileOverlay}>
     <Image
      source={{
       uri: "https://images.unsplash.com/photo-1457449940276-e8deed18bfff",
      }}
   ...
   <View style={styles.card}>
    ...
    <View style={styles.badgeRow}>
     {badgesData.map((badge, index) => (
      <Animated.View
       key={badge.id}
       style={[styles.badgeItem, animatedBadgeStyle(index)]}
      >
       <Ionicons
        name={badge.icon}
        size={40}
        color={badge.earned ? "#FF9F1C" : "#ccc"}
       />
    ...
    >
     {tripsData.map((trip, index) => (
      ...
      >
       <Animated.View
        key={trip.id}
        style={[styles.tripItem, animatedTripStyle(index)]}
       >
