...
export default function CreateTripScreen() {
  const [tripData, setTripData] = useState({
    ...
    travelFrom: "",
    destinations: [{ name: "" }],
    itinerary: [{ date: new Date(), activities: [{ title: "", description: "", location: "" }] }],
    numberOfPeople: "",
    images: [],
    primaryImageIndex: 0,
    price: "",
  ...
  const addItineraryDay = () => {
    setTripData({
      ...
      itinerary: [
        ...tripData.itinerary,
        { date: new Date(), activities: [{ title: "", description: "", location: "" }] },
      ],
    });
  };
  ...
  const addActivity = (dayIndex) => {
    ...
    newItinerary[dayIndex].activities.push({ title: "", description: "", location: "" });
        ...
        <View style={styles.contentArea}>
          ...
          <Animated.View entering={FadeInUp.delay(400)} style={styles.formSection}>
            ...
            {tripData.itinerary.map((day, dayIndex) => (
              <View key={dayIndex} style={styles.itineraryDay}>
                ...
                {day.activities.map((activity, actIndex) => (
                  <View key={actIndex} style={styles.activitySection}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(text) => updateActivity(dayIndex, actIndex, "title", text)}
                    />
                    <TextInput
                      style={[styles.input, styles.multilineInput]}
                      placeholder="Description"
                      ...
                      onChangeText={(text) => updateActivity(dayIndex, actIndex, "description", text)}
                    />
                    <View style={styles.dropdownContainer}>
                      <TouchableOpacity
                        style={styles.dropdownButton}
                            ...
                            <View key={index}>
                              ...
                              {loc.cities.map((city, cityIndex) => (
                                <TouchableOpacity
                                  ...
                                  style={[styles.dropdownItem, styles.dropdownSubItem]}
                                  onPress={() => {
                                    updateActivity(dayIndex, actIndex, "location", `${loc.state}, ${city}`);
                                    toggleActivityDropdown(dayIndex, actIndex);
                                  }}
                                >
                                  <Text style={styles.dropdownItemText}>{city}</Text>
                ...
                <TouchableOpacity style={styles.addButton} onPress={() => addActivity(dayIndex)}>
                  <Ionicons name="add" size={20} color="#F57C00" />
          ...
          <Animated.View entering={FadeInUp.delay(500)} style={styles.formSection}>
            ...
            <TextInput
              ...
              onChangeText={(text) => handleInputChange("additionalInstructions", text)}
            />
          </Animated.View>
